...
pid_t
getpgrp() /* POSIX.1 version */
{
	STUB_0(getpgrp);
}

pid_t
getpgrp(pid_t pid) /* BSD version */
{
	(void)pid;
	STUB_0(getpgrp);
}
...
int
setpgrp(pid_t pid, pid_t pgid) /* BSD version */
{
	(void)pid;
	(void)pgid;
	STUB_0(setpgrp);
...
int
uni_open(const char *filename, unsigned oflag, int mode)
{
	return _open(filename, oflag, mode);
}

int
mkdir2(const char *path, int mask)
{
	(void)mask;
	return _mkdir(path);
}
...
int
uni_open(const char *filename, unsigned oflag, ...)
{
	return _open(filename, oflag, 0);
}

int
fcntl(int handle, int mode, ...)
{
	(void)handle;
	(void)mode;
	STUB_0(fcntl);
...
size_t
unistd_safe_strlen(const char *s)
{
	if (!s) {
...
int
uni_sscanf(char *input, const char *format, ...)
{
