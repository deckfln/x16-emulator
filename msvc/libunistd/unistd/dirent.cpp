...
int
readdir_r(DIR *dir, struct dirent *, struct dirent **entry)
{
	errno = 0;
	if (!dir) {
		errno = EFAULT;
...
int
alphaqsort(const void *v1, const void *v2)
{
	dirent    **d1    = (dirent **)v1;
	dirent    **d2    = (dirent **)v2;
...
unsigned
GetFileCount(DIR *dir, scandir_f selector)
{
	unsigned count = 0;
	dirent   entry;
	dirent  *p = &entry;
...
int
scandir(const char *path, dirent ***namesList, scandir_f selector, scandir_alphasort sorter)
{
